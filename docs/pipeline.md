...
R16 实机弱网请求暴露出武士首开等待过长，因此默认构建还会生成独立的
`model.mobile.draco.glb` / `index.mobile.json`：512px 贴图，3.91 MB，
比 7.08 MB 完整资源减少约 45%，不简化几何、不修改节点图。
256px 试验仅降到 2.94 MB，因此选择 512px 保留放大观察所需的纹理。
手机、节省流量或显式省电模式优先使用轻量版本；精细模式载入完整版本。
切换时以稳定源节点 URL 恢复当前零件，回归测试检查真实资源请求和节点一致性。
