...
**R15：工程档案界面升级已于 2026-09-07 发布，并通过线上复查。**

发布代码：`c7d8b55`；[GitHub Pages 发布记录](https://github.com/BozhengLong/ironman-armor-lab/actions/runs/34109288669)。
CI 构建与浏览器验收通过；线上 HTML、CSS、JS 与构建产物哈希一致。
三个模型、标注钻取、逐件导航、材质分享恢复及 375×667 手机布局复查通过，未发现页面或资源请求错误。

- 新增 `web/presentation.js` / `.css`，负责展示材质、细轮廓、实际部件锚点引线与响应式布局。
- 按可见部件的当前投影取景，替换静态全局包围盒估算；手机面板展开后自动让出画布空间。
- SETTINGS 收纳算法与显示设置，SOURCE 恢复源材质；显示设置加入分享链接。
- 标签、触屏逐件按钮和双指缩放可操作；用户的鼠标、键盘或滑块操作会中断导览。
- 修复深链钻取未恢复幽灵透明度，以及组装没有使用已有逐件时间轴的问题。
- 静态检查覆盖外部模块；运行时验收新增标注及失效对照、逐件导航、显示链接、窄屏布局、
  组装错峰实际位移、发射完成和导览中断。原有绑定与署名等校验继续保留。

逐批记录见 `HANDOFF.md`（R1–R15）。本地开发服务器为 `http://localhost:8732/`。
