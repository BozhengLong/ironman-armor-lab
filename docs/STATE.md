...
**R16 进行中（2026-09-08）：** 五项优化已实现，正在完成持续运行及 iPhone 实机验收。
- 分阶段拆解、可暂停/前后切换的中文讲解、实际节点定位与按住原位对照。
- 漆面/金属区分与摄影棚反射；源材质切换仍可恢复。
- 按需重绘、画质自动降档、手机横屏布局；新增 `scripts/test_study.mjs` 纳入 CI。
- 已连接用户提供的 iPhone 16 Pro Max（iOS 26.6.1）。旧版实机静止场景约 48–51 FPS，
  新版实机数据尚待采集。不能把桌面测试写成手机结论。
