...
上一批（R14）已完成**并已发布**：把页面级校验放进 CI。此前 CI 从头到尾
没有任何一步解析过页面的 JS —— 实测注入一个必然白屏的语法错误，
`npm test` 与 `build_site.mjs` 双双退出码 0，CI 全绿。现在 `npm test` 含
`check_page.mjs`（语法 + DOM id 引用 + 占位符），CI 另起无头浏览器跑
`test_page.mjs`（五个 `__verify*` 钩子 × 三模型 × 两视口 + 深链往返 +
每条校验的对照组）。全程约 18 秒。

顺带修掉两个由此暴露的真实缺陷：URL 回写用的是防抖，而相机逐帧渐近插值让
OrbitControls 几乎每帧发 `change`，**防抖被无限顺延、场景动着时 URL 永不更新**
（LINK 可能复制到过期地址）；以及 agent 命令 `setExplode` 与旧钩子 `__setMode`
绕开了 URL 回写 —— 四条爆炸路径、两条模式路径已收敛到单一入口。

线上已复验两处修复：agent 命令改动会回写 URL（`mode` 与 `ex` 都出现在地址里）；
相机补间途中取到的链接同样带着当时的角度，重进能还原（178.2° -> 179.9°，
差值来自取整与阻尼，在容差内）。本次发布是第一次**先过运行时验收再上线**。

（R13）已完成并已发布：重复体标签一致性不变量，并借此安全地修掉
`MATERIAL_HINTS` 的子串缺陷。samurai 有 2 件由 `helmet` 纠正为 `neck`（有渲染核对），
另两个模型分类零变化。线上已确认 `samurai.json` 里 node 54/56 都是 `neck/C`
（孪生件未被劈开），samurai 组数 11，三个模型的各项校验全部通过。

**中断点：无进行中的工作，未决清单为空。**

（R10–R12）已完成并已发布：可分享的深链与社交预览、
`prefers-reduced-motion`、Agent 导览、衬底覆盖检查，以及朝前判定的真值核对
（结论是删掉两条不可靠的启发，而不是补第四条）。
`assets/manifests` 的改动只在 `frame` 元数据，分类与位移向量逐字节未变。

线上已验：深链往返逐字段一致、og 元标签与 1200×630 预览图可达（200）、
占位符无残留、导览可跑可中断、`prefers-reduced-motion` 生效（组装直接到终态）、
七层校验中可在浏览器里跑的五项全部通过。

（R9）已完成并已发布：REPULSOR 动作，以及 CI 补上 samurai 的
...
备份异地与 token 轮换都已明确决定不做（见未决事项 2、3），不要再提。
剩下的都是可选项，没有一项处于「不做就错着」的状态：

R10–R12 已把当时列的都做完（深链 + 社交预览、减少动态效果、Agent 导览、
朝前判定核对）。**当前没有已知的待办**，剩下的都是新想法而非欠债。

仍然留着的一处遗留：`MATERIAL_HINTS` 的子串匹配缺陷，见未决事项 5。
