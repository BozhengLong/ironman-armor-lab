...
上一批（R10–R12）已完成**并已发布**：可分享的深链与社交预览、
...
**遗留**：`analyze_parts.py` 的 `MATERIAL_HINTS` 仍是子串匹配，
`face` 会命中 `aiStandardSurface`。试修后发现会把一对完全相同的重复零件
劈成两个标签，比一致地错更糟，故按兵不动 —— 理由与证据见 `explode-design.md`。
