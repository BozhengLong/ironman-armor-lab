...
R17 已发布（代码 `b850363`）：主展区聚焦钢铁侠，武士移至非 Marvel 实验展区；
新增带来源的装甲档案和按模型编写的导览。移除来源未确认的 Mark 型号标签。
本轮变更与验证见 HANDOFF.md 顶部。
[完整 CI 与发布记录](https://github.com/BozhengLong/ironman-armor-lab/actions/runs/34192826750) 均成功；
线上 HTML、CSS、模型清单与本地构建哈希一致，线上档案→导览通过，无页面或控制台错误。
...
- 真实 iPhone 16 Pro Max / iOS 26.6.1：三套模型的阶段拆解、定位、原位对照、导览控制及
  ...
  后导览暂停、渲染计数不变，返回后可继续。控件由 DOM 事件驱动；用户随后确认 iPhone 优化可以告一段落，实机工作已结束。
