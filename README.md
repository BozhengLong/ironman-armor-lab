...
手机支持单指旋转、双指缩放，取景会随详情和设置面板变化，并有独立的横屏布局。

钻取后可在 **原位 / 大件 / 细件** 三个阶段间切换。在推荐的 3/4F 角度，
展开位置根据真实投影包围盒排列，以减少互相遮挡；部件的形状、比例和朝向不变。
**整体位置图** 标出所选部件原来的位置，按住 **查看原位**（键盘可按住空格或 Enter）
可对照，松开返回拆解状态。源节点、三角面和材质名收纳在 **模型数据** 中。
这些是按源网格组织的观察部件，不代表真实机械层级。

**TOUR** 提供中文分步讲解，支持暂停、继续、前后切换和结束。
暂停同时冻结画面与讲解进度；切到后台自动暂停，回到页面后手动继续。
手动旋转或操作其他控件可接管场景。
...
`finish=source`、`labels=0`、拆解 `stage=0|1|2` 可随分享链接还原。
新链接的 `node` 固定到源节点编号；旧链接只有 `p` 时仍按旧的钻取排序还原。

画质可选 **自动 / 省电 / 均衡 / 精细**。自动模式按设备和持续帧耗时降档；
省电模式限制到 1× 分辨率，仅为聚焦件显示边线，均衡最高 1.5×，精细最高 2×。
手机默认使用武士的轻量贴图资源（3.91 MB，完整资源为 7.08 MB），几何与节点不变。
选择“精细”会载入高清资源并恢复当前零件；画质选择也保存在链接中。
静止时停止 3D 帧循环，交互时恢复；讲解停留只更新进度条。遥测中的 REST 表示休眠，
并非持续渲染的帧率。
...
npm run test:page # 布局、标注、钻取、动画、链接、阶段几何、定位、休眠与对照组检查
...
页面把「你看到的是什么」编码进 URL：模型、视角方位角与俯仰角、正交缩放、
爆炸进度、爆炸模式、钻取的组、聚焦的零件、碎石开关。HUD 上的 `LINK` 按钮
复制当前视图的地址。

```
?model=hulkbuster&az=90.0&el=0.0&ex=0.620&mode=semantic&g=forearm%2FL&p=2&debris=1
```

用 `replaceState` 而不是 `pushState` —— 拖动相机会产生几百个中间状态，
进历史记录会把后退键变成噪音。每一项在还原时都先校验再用：链接是外部输入，
坏值退回默认而不是让场景崩掉。

站点还带 Open Graph / Twitter Card 元标签与 1200×630 预览图。
`og:image` 需要绝对地址，而绝对地址属于部署信息，因此源文件里写占位符
`__SITE_URL__`，由 `build_site.mjs` 在出站点时替换 —— 占位符或预览图缺失
都会让构建失败，因为「分享出去没有预览图」恰恰是没人会主动去测的那类问题。

动画遵循 `prefers-reduced-motion`：组装直接给终态，发射保留充能与放电、
去掉手臂后坐（发光不是前庭刺激源，位移才是），视角切换与爆炸补间改为瞬时。
